...
let getExtraInfoStaffById = async (req, res) => {
  try {
    let info = await staffService.getExtraInfoStaffById(req.query.staffId)
    return res.status(200).json(info)
  } catch (error) {
    console.log(error)
    return res.status(200).json({
...
let getProfileStaffById = async (req, res) => {
  try {
    let info = await staffService.getProfileStaffById(req.query.staffId)
    return res.status(200).json(info)
  } catch (error) {
    console.log(error)
    return res.status(200).json({
...
let sendRemedy = async (req, res) => {
  try {
    let info = await staffService.sendRemedy(req.body)
    return res.status(200).json(info)
  } catch (error) {
    console.log(error)
    return res.status(200).json({
